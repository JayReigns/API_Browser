...
class API_PT_Browser(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator(API_OT_GOTO_Module.bl_idname,
                     text="Parent", icon="BACK").info = '-1 0'
        row.operator(API_OT_GOTO_Module.bl_idname, text='bpy',
                     emboss=True, icon="FILE_PARENT").info = '-2 0'
        ...
        for i, category in enumerate(data_tree):
            ...
            for j, entry in enumerate(category):
                ...
                row.operator(API_OT_GOTO_Module.bl_idname, text=str(
                    entry), emboss=True).info = f'{i} {j}'
